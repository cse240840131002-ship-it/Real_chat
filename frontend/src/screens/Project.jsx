...
const Project = () => {
    ...
    return (
        <div className='overflow-auto bg-slate-950 text-white rounded-sm p-2'>
            <Markdown
                children={parsed?.text || "No response"}
                options={{
                    overrides: {
    ...
    if (data.sender._id === 'ai') {
        ...
        try {
            ...
            console.error("Invalid JSON from AI:", data.message);
        ...
        // ✅ Only mount if fileTree exists
        if (message?.fileTree) {
            webContainer?.mount(message.fileTree);
            setFileTree(message.fileTree);
        }

        // ✅ IMPORTANT: store parsed message (not raw)
        setMessages(prev => [
            ...prev,
            { ...data, message }
        ]);
